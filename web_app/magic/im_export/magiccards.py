...
def import_card_image(card):
    ...
    url = '{domain}/search?{params}'.format(
        domain=magiccards_domain, params=params)
    ...
    if len(img_urls) == 0:
        raise MagicCardImageImportException(
            "Could not download image from: {}. May be the card name doesn't match exactly".format(url))
    elif len(img_urls) == 1:
        img_url = img_urls[0]
        return img_url
    else:
        raise MagicCardImageImportException("Could not download image from: {}. To many results!".format(url),
