...
def card_img(card, args=None):
    ...
    """
    assert isinstance(card, CardState), 'can only handle instances of {}, but received {}'.format(CardState, card.__class__)
    h = w = 0
    ...
    return mark_safe("<div class='card-img-wrapper' ><img src='{}' alt='{}' {} {} class='hover-enlarge' id={}/></div>"
                     .format(card.image.url, card.name, height, width, card.external_id))
