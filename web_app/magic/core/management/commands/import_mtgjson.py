...
class Command(BaseCommand):
    all_cards_json = os.path.join(
        settings.PROJECT_DIR, "data/mtgjson.com/AllCards.json"
    )
    ...
    def handle(self, *args, **options):
        if options["cards"]:
            mtgjson.import_cards(
                json.load(open(self.all_cards_json, "r")),
                verbosity=options["verbosity"],
            )
        elif options["sets"]:
            mtgjson.import_sets(
                json.load(open(self.all_sets_json, "r")), verbosity=options["verbosity"]
            )
        else:
