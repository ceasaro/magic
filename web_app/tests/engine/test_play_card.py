...
def test_play_land_card(player):
    ...
    with pytest.raises(MagicGameException):
        player.play(forest)
        assert player.hand.count() == 3, "no changes expectd casuse card was not in hand"
        assert player.played_cards.count() == 1, "no changes expectd casuse card was not in hand"
        assert str(player.mana_pool) == '0', "no changes expectd casuse card was not in hand"
