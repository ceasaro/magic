...
def test_get_deck(api_client_cees, deck):
    ...
    assert all([key in keys for key in ['name', 'external_id', 'mana_cost', 'image_url']])
...
def test_create_deck1(api_client_cees, card_library):
    ...
    assert all([id in card_ids for id in response.json()['cards']]), "response must contain all card_ids"
...
def test_create_deck_with_same_cards(api_client_cees, card_library):
    ...
    response = api_client_cees.post('/api/decks/', {'name': 'deck with same cards',
                                                    'cards': card_ids})
    assert response.status_code == 201
    deck = Deck.objects.first()
    assert len(deck.cards.all()) == 2, "Deck should contain two cards"
