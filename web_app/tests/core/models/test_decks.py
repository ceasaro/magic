...
@pytest.mark.parametrize(("search_query", "found_cards"), [
    (None, 3),
    ('', 3),
    ('test', 2),
    ('test deck2', 1),
...
def test_search_deck(decks, search_query, found_cards):
    assert Deck.objects.search(search_query).count() == found_cards, \
        "search query '{}' search should return {} cards".format(search_query, found_cards)
