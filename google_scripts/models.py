...
class MtgStatsManager(object):
    ...
    def _from_google_values(self, google_data):
        ...
        if header_row != self.HEADER_ROW:
            raise MtgStatsException(
                "Spreadsheet header differs from expected header, can't load data. {} != {}.".format(
    ...
    def load(self, overwrite=False):
        if self.raw_data and not overwrite:
            raise MtgStatsException(
                "Data already loaded from google, use the 'overwrite' parameter to load anyway"
            )
