...
class Client(object):
    ...
    def get_file(self):
        result = self.sheet_service.spreadsheets().values().get(spreadsheetId=self.google_file_id,
                                                                range=MTG_SPREADSHEET_RANGE).execute()
    ...
    def store_file(self, values):
        ...
        result = self.sheet_service.spreadsheets().values().update(spreadsheetId=self.google_file_id,
